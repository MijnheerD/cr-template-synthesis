...
coefX, dX = amplitude_fit_slice(freq[frange],  filtered[:, 0], Xslice, distances[antenna], f0=50)
coefY, dY = amplitude_fit_slice(freq[frange],  filtered[:, 1], Xslice, distances[antenna], f0=50)

qoefX, qX = amplitude_fit_slice_iter(freq[frange],  filtered[:, 0], Xslice, distances[antenna], f0=50)
qoefY, qY = amplitude_fit_slice_iter(freq[frange],  filtered[:, 1], Xslice, distances[antenna], f0=50)
...
ax.plot(freq[frange]/1e6, amplitude_fun_slice(freq[frange]/1e6, (coefX[0]), coefX[1], coefX[2], dX, f0=50),
        color='orange', label='My fit')
ax.plot(freq[frange]/1e6, amplitude_fun_slice(freq[frange]/1e6, (qoefX[0]), qoefX[1], qoefX[2], qX, f0=50),
        color='yellow', label='Iterative fit')
...
ax2.plot(freq[frange]/1e6, amplitude_fun_slice(freq[frange]/1e6, (coefY[0]), coefY[1], coefY[2], dY, f0=50),
         color='blue', label='My fit')
ax2.plot(freq[frange]/1e6, amplitude_fun_slice(freq[frange]/1e6, (qoefY[0]), qoefY[1], qoefY[2], qY, f0=50),
         color='lightblue', label='Iterative fit')
